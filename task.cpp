...
static const uint64_t MESSAGE_COUNT = 500;   // test size
static const uint64_t STATS_SAMPLE_SIZE = 100; // sample size for stats
...
public:
  ...
  int operator()(int & buffer, int count) {
    ...
    std::cout << "num_rx=" << num_rx << "\n";

    // fill data
    value_ += (++sent_) + 1;
    buffer = abs(value_) % MAX_INPUT;
    return num_rx;
  }
};
