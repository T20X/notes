...
- if T is a (possibly cv-qualified) class type, the constructors are considered and subjected to overload resolution against the empty argument list. The constructor selected (which is one of the default constructors) is called to provide the initial value for the new object; NOTE THAT FOR CLASS TYPE IF NO DEFAULT CONSTRUCTOR CAN BE FOUND THEN IT WOULD COMPILE ERROR!
- ELSE if T is an array type, every element of the array is default-initialized;
- else otherwise, no initialization is performed: the objects with automatic / dynamic storage duration (and their subobjects) contain indeterminate values
...
() style direct initialization is more permissive as it allows narrowing conversations and using explicit constructors
...
A class type T is const-default-constructible if default initialization of T would invoke a user-provided constructor of T (not inherited from a base class) (since C++11) 
...
T object {};	(4)	(since C++11) - BE CAREFULL FOR AGGREGATES IT WOULD CALL AGGREGATE INITIALIZATION which has these rules so explicit defult constructors may not work!
...
Turns into aggregate initialization: In all cases, if the empty pair of braces {} is used and T is an aggregate type, aggregate-initialization is performed instead of value-initialization.
...
class S{ std::string s; 
S():s("sadf")}; <-------------- member initializer list 
int n = 7; <---------  default member initialize
...
std::allocator_traits<std::allocator>::allocate calls std:;allocator<T>::allocate which in turn would could some form of ::operator new (new /ext/new_allocator.h does that)
allocator_traits<std::allocator>::deallocate calls std::allocator<T>::deallocate
...
gcc using in vector _gnu_cxx::_allocator_traits which is based on std::allocator_traits
it is using  alloc traits from gcc to allocate / deallocae its internal storage
...
brace initialization extends ref lifetime
...
C-style cast
----------------------
It would attempt to do other casts in the following order:
...
/* Also no issue with aliasing here, object lifetime has been started with placement new */
...
it really does this under the hood
auto _e_hidden_ = f();
...
Then compiler wont even look at f() from above, but rather would execute other::f;
-------------
...
In ISO C, it does not provide an external definition at all. In ISO C++ it does provide an external definition; however C++ has an additional rule (which C doesnt), that if there are multiple external definitions of an inline function,
...
extern inline void f(void); in a .c file in ISO C is meant to be paired with the use of inline void f(void) {} in header files. It causes the external definition of the function to be emitted in that translation unit.
 If you dont do this then there is no external definition, and so you may get a link error (it is unspecified whether any particular call of f links to the external definition or not).

In other words, in ISO C you can manually select where the external definition goes; or suppress external definition entirely by using static inline everywhere;
...
dont mix unsigned with singed with singed! int a;unsigned int b;
...
std::decay
-------------
Basically, its this, in pseudo-code:
